...
void MD5::intstate(uint32_t retstate[])
{
    int i;
    if (!finalized) {
        for (i=0; i<4; i++) {
...
void md5state(const std::string str, uint32_t outstate[])
{
    MD5 md5 = MD5(str);
    md5.intstate(outstate);
    return;
}
///////////////////////////////
...
XORSHIFT128::XORSHIFT128(const uint32_t *state)
{
    init(state[0], state[1], state[2], state[3]);
}


void XORSHIFT128::init(uint32_t a,
                       uint32_t b,
                       uint32_t c,
                       uint32_t d)
{
...
void XORSHIFT128::seed(const uint32_t *state)
{
    x = state[0];
    y = state[1];
    z = state[2];
...
uint32_t XORSHIFT128::randint(uint32_t max) {
    t = x;
    t ^= t << 11;
    t ^= t >> 8;
    x = y; y = z; z = w;
...
///////////////////////////////////////
// END of XORSHIFT128 implementation //
///////////////////////////////////////
...
FP::FP(const uint32_t fpsize)
{
    init(fpsize);
}

void FP::init(const uint32_t fpsize)
{
    size = fpsize;
    if ((size % 32) == 0)
    {
...
void FP::digest(const std::string &sequence)
{
    ...
    numAA = sequence.length();
    substrings.resize(numAA);
    counts.resize(numAA);
    
    for (sublen = 2; sublen <= MAXSHORTWORDLEN; sublen++)
    {
        count = 0;
        for(i = 0; i < (numAA - sublen + 1); i++) {
            tmpstring = sequence.substr(i, sublen);
            found = false;
            for(j = 0; j < count; j++) {
                if (substrings[j] == tmpstring) {
                    if (counts[j] < MAXONBITS)
                    {
                        counts[j] += 1;
                    }
                    found = true;
            ...
            if (!found) {
                substrings[count] = tmpstring;
                counts[count] = MINONBITS;
                count += 1;
            }
        }
        for (i=0; i < count; i++)
        {
            update(substrings[i], counts[i]);
        }
    }
    to_int();
}

void FP::update(const std::string &substring, uint32_t count)
{
    uint32_t i;
    uint32_t pos;
    md5state(substring, state);
    random.seed(state);
    for (i=0; i < count; i++)
    {
        pos = random.randint(size);
        fp.set(pos);
    }
}
...
////////////////////////////////////////
// END of fingerprints implementation //
////////////////////////////////////////
